...
SHOW_DEMO_CREDENTIALS = (
    os.getenv("SHOW_DEMO_CREDENTIALS", "true").lower() == "true"
...
AUTH_MAX_RECENT_FAILURES = int(
    os.getenv("AUTH_MAX_RECENT_FAILURES", "5")
)  # Max failures per window
AUTH_RATE_LIMIT_WINDOW_MINUTES = int(
    os.getenv("AUTH_RATE_LIMIT_WINDOW_MINUTES", "15")
)  # Window in minutes
AUTH_LOCKOUT_THRESHOLD = int(
    os.getenv("AUTH_LOCKOUT_THRESHOLD", "10")
)  # Total failures before lockout
AUTH_CLEANUP_DAYS = int(os.getenv("AUTH_CLEANUP_DAYS", "30"))  # Days to keep failed attempts

# Input Validation
MAX_RESUME_LENGTH = int(
    os.getenv("MAX_RESUME_LENGTH", "100000")
)  # Max resume size in characters (100KB)
MAX_JOB_DESCRIPTION_LENGTH = int(
    os.getenv("MAX_JOB_DESCRIPTION_LENGTH", "50000")
)  # Max job description size
