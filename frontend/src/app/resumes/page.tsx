...
export default function ResumesPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      {/* Tab Content */}
      {activeTab === 'list' && <ResumeListTab token={tokens.access_token} />}
      {activeTab === 'analysis' && <ATSAnalysisTab token={tokens.access_token} />}
      {activeTab === 'keywords' && <KeywordGapTab token={tokens.access_token} />}
      {activeTab === 'templates' && <TemplatesTab token={tokens.access_token} />}
    </div>
  )
}
...
function ResumeListTab({ token }: { token: string }) {
  ...
  const deleteResume = async (id: number) => {
    ...
    try {
      await resumesApi.delete(token, id)
...
function ATSAnalysisTab({ token }: { token: string }) {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const result = await resumesApi.upload(token, file)
  ...
  const analyzeResume = async () => {
    ...
    try {
      const result = await resumesApi.analyze(token, {
  ...
  const optimizeResume = async () => {
    ...
    try {
      const result = await aiApi.tailorResume(token, {
        resume_content: resumeText,
...
function KeywordGapTab({ token }: { token: string }) {
  const [resumeText, setResumeText] = useState('')
  const [jobDescription, setJobDescription] = useState('')
  ...
  const getAISuggestions = async () => {
    ...
    try {
      const result = await aiApi.optimizeResume(token, {
        resume_content: resumeText,
        job_description: jobDescription,
      })
                ...
                {formData.education.map((edu, index) => (
                  <div key={edu.id} className="p-4 border border-gray-200 rounded-lg">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          <label className="block text-xs font-medium text-gray-600">Graduation Date</label>
                          <input
                            type="text"
        ...
        <div className="bg-white rounded-lg shadow p-6">
          <div className="flex justify-between items-center mb-4">
            ...
            <div className="flex gap-2">
              <button
                onClick={() => {
                  navigator.clipboard.writeText(generateResumeContent())
                }}
                className="inline-flex items-center px-3 py-1.5 text-sm border border-gray-300 rounded-md hover:bg-gray-50"
...
function ResumeFormModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (resume) {
        savedResume = (await resumesApi.update(token, resume.id, formData)) as Resume
      } else {
        savedResume = (await resumesApi.create(token, formData)) as Resume
      }
      onSave(savedResume)
    } catch (err) {
      console.error('Failed to save resume:', err)
