...
from models.database import get_db_connection, get_or_create_profile_for_user, init_database
...
def cleanup_test_data():
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "DELETE FROM users WHERE username LIKE 'testuser%' OR email LIKE 'test%@example.com'"
        )
    ...
    try:
        ...
        cursor.execute(
            "DELETE FROM job_applications WHERE profile_id IN (SELECT id FROM profiles WHERE user_id IN (SELECT id FROM users WHERE username LIKE 'testuser%'))"
        )
        cursor.execute(
            "DELETE FROM resumes WHERE profile_id IN (SELECT id FROM profiles WHERE user_id IN (SELECT id FROM users WHERE username LIKE 'testuser%'))"
        )
        cursor.execute(
            "DELETE FROM cover_letters WHERE profile_id IN (SELECT id FROM profiles WHERE user_id IN (SELECT id FROM users WHERE username LIKE 'testuser%'))"
        )
        cursor.execute(
            "DELETE FROM career_journal WHERE profile_id IN (SELECT id FROM profiles WHERE user_id IN (SELECT id FROM users WHERE username LIKE 'testuser%'))"
        )
        cursor.execute(
            "DELETE FROM profiles WHERE name LIKE 'Test User%' OR email LIKE 'test%@example.com'"
        )
        conn.commit()
        conn.close()
    except Exception as e:
...
def main():
    ...
    try:
        ...
        print_test(
            "Authenticate with valid credentials", user is not None, f"User: {user['username']}"
        )
        if user:
            passed_tests += 1
    except Exception as e:
    ...
    try:
        ...
        print_test(
            "Get existing profile (no duplicate)", same_profile, f"Profile ID: {profile1['id']}"
        )
        if same_profile:
            passed_tests += 1
    except Exception as e:
