...
This migration creates the initial schema for ResuBoost AI.
It includes all existing models:
...
- CareerJournalEntry: Career achievements journal
- JobAlert: Real-time job notifications
- CompanyFilter: Company blacklist/whitelist
- KeywordFilter: Keyword filtering for jobs
- ApplicationQuestion: Pre-filled application answers
"""
...
def upgrade() -> None:
    ...
    op.create_index('ix_job_applications_company_position', 'job_applications', ['company', 'position'], unique=False)
    ...
    op.create_table(
        ...
        sa.Column('filter_type', sa.Enum('blacklist', 'whitelist', name='companyfilterenum'), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column('filter_type', sa.Enum('exclude', 'require', name='keywordfilterenum'), nullable=False),
        sa.Column('applies_to', sa.Enum('title', 'description', 'both', name='keywordappliestoenum'), nullable=False),
        sa.Column('created_at', sa.DateTime(), nullable=True),
        sa.ForeignKeyConstraint(['user_id'], ['users.id'], ondelete='CASCADE'),
        sa.PrimaryKeyConstraint('id')
    )
    ...
    op.create_table(
        ...
        sa.Column('question_type', sa.Enum('text', 'number', 'select', 'boolean', name='questiontypeenum'), nullable=False),
...
def downgrade() -> None:
    ...
    op.drop_index('ix_job_applications_company_position', table_name='job_applications')
    ...
    op.execute("DROP TYPE IF EXISTS keywordappliestoenum")
    op.execute("DROP TYPE IF EXISTS keywordfilterenum")
    op.execute("DROP TYPE IF EXISTS companyfilterenum")
