...
with tab2:
    ...
    with col2:
        data["location"] = st.text_input(
            "Location", value=data.get("location", ""), placeholder="City, State"
        )
        data["linkedin"] = st.text_input(
            "LinkedIn URL", value=data.get("linkedin", profile.get("linkedin", ""))
        )
        data["github"] = st.text_input(
            "GitHub URL", value=data.get("github", profile.get("github", ""))
        )

    data["portfolio"] = st.text_input(
        "Portfolio URL", value=data.get("portfolio", profile.get("portfolio", ""))
    )
    ...
    if "experience" in template["sections"]:
        ...
        num_exp = st.number_input(
            "Number of positions", 1, 4, len(data["experiences"]), key="num_exp"
        )
            ...
            with st.expander(f"Position {i+1}", expanded=(i == 0)):
                ...
                with col1:
                    exp["title"] = st.text_input(
                        "Job Title", value=exp.get("title", ""), key=f"exp_title_{i}"
                    )
                ...
                with col2:
                    exp["location"] = st.text_input(
                        "Location", value=exp.get("location", ""), key=f"exp_loc_{i}"
                    )
            ...
            with st.expander(f"Education {i+1}", expanded=(i == 0)):
                ...
                with col1:
                    ...
                    edu["school"] = st.text_input(
                        "School", value=edu.get("school", ""), key=f"edu_school_{i}"
                    )
                with col2:
                    edu["graduation"] = st.text_input(
                        "Graduation Date", value=edu.get("graduation", ""), key=f"edu_grad_{i}"
                    )
                    edu["gpa"] = st.text_input(
                        "GPA (optional)", value=edu.get("gpa", ""), key=f"edu_gpa_{i}"
                    )
                edu["honors"] = st.text_input(
                    "Honors (optional)", value=edu.get("honors", ""), key=f"edu_honors_{i}"
                )
    ...
    if "projects" in template["sections"]:
        ...
        num_proj = st.number_input(
            "Number of projects", 1, 4, len(data["projects"]), key="num_proj"
        )
            ...
            with st.expander(f"Project {i+1}", expanded=(i == 0)):
                ...
                with col1:
                    proj["name"] = st.text_input(
                        "Project Name", value=proj.get("name", ""), key=f"proj_name_{i}"
                    )
                    proj["tech"] = st.text_input(
                        "Technologies", value=proj.get("tech", ""), key=f"proj_tech_{i}"
                    )
                with col2:
                    proj["link"] = st.text_input(
                        "Link (optional)", value=proj.get("link", ""), key=f"proj_link_{i}"
                    )
            ...
            try:
                with get_db_connection() as conn:
                    ...
                    """,
                        (
                            profile["id"],
                            f"{template_name} - {datetime.now().strftime('%Y-%m-%d')}",
                            resume_text,
                        ),
                    )
                    conn.commit()
